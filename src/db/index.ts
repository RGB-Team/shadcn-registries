...
export const getPaginatedRegistries = (
  ...
  if (dataParams && Object.keys(dataParams).length >= 1 ) {
    ...
    console.log('From Date:', fromDate ? format(fromDate, 'yyyy-MM-dd') : 'Not set');
    console.log('To Date:', toDate ? format(toDate, 'yyyy-MM-dd') : 'Not set');
    ...
    targetRegistries = targetRegistries.filter((reg) => {
      ...
      if (fromDate && toDate) {
        ...
        result = isAfter(createdAt, fromDate) || createdAt.getTime() === fromDate.getTime();
      } else if (toDate) {
        result = isBefore(createdAt, toDate) || createdAt.getTime() === toDate.getTime();
      }

      console.log('Include item:', result);
      return result;
    });

    //console.log('Filtered registries count: , and the value:', targetRegistries.length, targetRegistries);
  }
  console.log('Filtered registries count:', targetRegistries.length);
